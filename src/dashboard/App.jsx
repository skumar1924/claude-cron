...
export function App() {
  ...
  const reload = useCallback(async () => {
    const [j, r] = await Promise.all([fetchJobs(), fetchRuns({ days: 30 })])
    setJobs(j)
    setRuns(r)
  ...
  const orphans = runs.filter(r => r.job_id === null)
  ...
  return (
    <div className="app">
      {orphans.length > 0 && (
        <div className="warning-banner">{orphans.length} untracked run(s) detected</div>
      )}
      ...
      <main className="detail">
        {showForm
          ...
          : <DetailPanel job={selectedJob} runs={runs.filter(r => r.job_id === selectedId)} onEdit={openEdit} onReload={reload} />
        }
      </main>
    </div>
    </div>
