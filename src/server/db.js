...
export function insertRun(db, { id, job_id, started_at, status = 'running' }) {
  db.prepare(`INSERT INTO runs (id, job_id, started_at, status)
              VALUES (?, ?, ?, ?)`).run(id, job_id ?? null, started_at, status)
}
